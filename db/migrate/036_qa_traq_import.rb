...
#  +-------------------+-----------------------+------+-----+---------+-------+
  ...
  def self.up
    ...
    g = Group.find_or_create_by_name('ECT')
    while row = @rs.fetch_hash do
      puts "importing #{row['TestCaseID']}..."
      user = User.find_or_create_by_login(:login => row['UserName'].downcase, :email => "#{row['UserName'].downcase}@expedia.com", :password => 'mttpower', :password_confirmation => 'mttpower', :group_id => g.id)
      user.activate unless user.activation_code.nil?
      category = Category.find_or_create_by_name(:name => 'ECT', :group_id => g.id).children.find_or_create_by_name(:name => row['ProductName'], :group_id => g.id).children.find_or_create_by_name(:name => row['ProductComponentName'], :group_id => g.id)
