...
class NestedSet < Mixin
  acts_as_enumerated_path :scope => "mixins.root_id IS NULL"
end

class NestedSetWithStringScope < Mixin
  acts_as_enumerated_path :scope => 'mixins.root_id = #{root_id}'
end
...
class NestedSetWithSymbolScope < Mixin
  acts_as_enumerated_path :scope => :root
end

class Category < Mixin
  acts_as_enumerated_path
end
