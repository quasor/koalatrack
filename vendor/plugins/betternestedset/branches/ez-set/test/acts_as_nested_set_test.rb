...
class MixinNestedSetTest < Test::Unit::TestCase
  ...
  def check_method_mixins(obj)
    [:<=>, :all_children, :ancestors, :before_create, :before_destroy, :check_full_tree, :check_subtree, :children, 
    :all_children_count, :full_set, :leaves, :leaves_count, :left_col_name, :level, :move_to_child_of, 
    :move_to_left_of, :move_to_right_of, :move_to_root, :parent, :parent_col_name, :renumber_full_tree, :right_col_name, 
  ...
  def test_protected_attributes
    ns = NestedSet.new(:lft => 3, :rgt => 2)
    [:lft, :rgt].each {|symbol| assert_equal(nil, ns.send(symbol))}
  end
  ...
  # test the callbacks that trigger updates of lft/rgt values after save  
  def test_after_save
    ns = set2(9)
    ns.parent=(set2(1))
    ns.save
