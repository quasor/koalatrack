...
#  ssl_required :show, :index
  ...
  def index
    ...
    if params[:q] && !params[:q].blank?
      ...
      @playlists = current_user.group.playlists.find(:all, :conditions => @conditions).reverse - @my_playlists
    else
      #login_required
  ...
  def show
    ...
    @summary = TestCaseExecution.find_by_sql "SELECT last_result as result, count(last_result) as total FROM playlist_test_cases JOIN playlists ON playlist_test_cases.playlist_id = playlists.id JOIN koala_test_cases ON playlist_test_cases.test_case_id = koala_test_cases.id WHERE (`koala_test_cases`.`active` = 1 AND `playlists`.`id` = #{@playlist.id}) GROUP BY last_result"

    if logged_in?
      @user_summary =  TestCaseExecution.find_by_sql "SELECT last_result as result, count(last_result) as total FROM playlist_test_cases JOIN playlists ON playlist_test_cases.playlist_id = playlists.id JOIN koala_test_cases ON playlist_test_cases.test_case_id = koala_test_cases.id WHERE (`koala_test_cases`.`active` = 1 AND `playlists`.`id` = #{@playlist.id} AND `playlist_test_cases`.`user_id` = #{current_user.id}) GROUP BY last_result"
    end
    ...
    sort = case params[:sort]
               when "feature"  then "koala_test_cases.priority_in_feature"
               when "product"  then "koala_test_cases.priority_in_product"
          ...
          if !params[:q].blank?
           sort = case params[:sort]
            ...
            with_ops = {:playlist_id => @playlist.id}
						with_ops.merge!({:test_case_executions_count => 0}) if session[:filtering]
